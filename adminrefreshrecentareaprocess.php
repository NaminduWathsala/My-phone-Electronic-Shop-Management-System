...
?>
        ...
        <a href="manageusers.php?email=<?php echo $u["from"]; ?>" class="list-group-item list-group-item-action active text-white rounded-0">
            <div class="media">
                <?php
                $profileimg = Database::search("SELECT * FROM `profile_img` WHERE `user_email` = '" . $mail . "' ");
                $pi = $profileimg->fetch_assoc();
                ?>
                <img src="<?php echo $pi["code"] ?>" alt=" user" width="50" class="rounded-circle">
